...
public class SwiftCodeControllerJD {
	...
	@GetMapping
	public ResponseEntity<Page<SwiftCodeDTO>> findBySpecification(@RequestParam(defaultValue = "0") int page_number,
			@RequestParam(defaultValue = "30") int page_size) {
	...
	public ResponseEntity<byte[]> exportSwiftCodesToExcel(@RequestBody SwiftCodeSearchDTO searchDTO) {
		try {
			...
			List<SwiftCodeEntity> swiftCodes = swiftCodeDTOs.stream().map(SwiftCodeIServiceJPADynamic::maptoEntity)
...
//    }
	...
	public ResponseEntity<?> importSwiftCodesFromExcel(@RequestParam("file") MultipartFile file) {
		try {
			Map<String, Object> result = service.importFromExcel(file.getInputStream());
			...
			if (errorFile != null) {
				return ResponseEntity.ok().header(HttpHeaders.CONTENT_DISPOSITION, "attachment; filename=errors.xlsx")
						.contentType(MediaType
								.parseMediaType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"))
						.body(errorFile);
			}
	...
	public ResponseEntity<Void> updateParaStatus(@PathVariable int id, @RequestParam int para) {
		try {
			this.service.updateParaStatus(id, para);
			return new ResponseEntity<>(HttpStatus.OK);
		} catch (Exception e) {
